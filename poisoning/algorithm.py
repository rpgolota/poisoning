...
class xiao2018:
    ...
    def projection(self, value):
        ...
        if type(value) is tuple and len(value) == 2:
            ...
            value = (
                (value[0], value[1]) if value[0] < value[1] else (value[1], value[0])
            )
    ...
    def _partial_derivatives(self, X, Y, ax, ay, weights, biases):
        ...
        mu = np.mean(X, axis=0)
        M = np.outer(ax, weights) + ((np.dot(weights, ax) + biases) - ay) * np.identity(
            n
        )
    ...
    def _gradient(self, X, Y, ax, ay):
        ...
        partial_weights, partial_biases = self._partial_derivatives(
            X, Y, ax, ay, weights, biases
        )
        last_term = self.alpha * (
            np.matmul(self._gradient_r_term(weights), partial_weights)
        )
        result = [
            ((np.dot(item[0], ax) + biases) - item[1])
            * (np.matmul(item[0], partial_weights) + partial_biases)
            for item in zip(X, Y)
        ]
    ...
    def _gradient_r_term(self, weights):
        ...
        if self.algorithm_type == "lasso":
            ...
            return (
                self.rho
                * np.array([-1 if i < 0 else 1 if i > 0 else 0 for i in weights])
                + (1 - self.rho) * weights
            )
    ...
    def _project(self, value):
        if self._projection_type == "vector":
            return np.array(
                [
                    v
                    if v >= proj[0] and v <= proj[1]
                    else min(proj, key=lambda val: abs(val - v))
                    for v, proj in zip(value, self._projection)
                ]
            )
        else:
            return np.array(
                [
                    v
                    if v >= self._projection[0] and v <= self._projection[1]
                    else min(self._projection, key=lambda val: abs(val - v))
                    for v in value
                ]
            )
    ...
    def _bounds(self, X, Y):
        ...
        return (1 / length) * sum(
            [(1 / 2) * ((np.dot(weights, x) - y) ** 2) for x, y in zip(X, Y)]
        ) + (self.alpha * self._regularize(weights))
    ...
    def _find_alpha(self, X, Y):
        ...
        if self.algorithm_type == "lasso":
            ...
            reg = linear_model.ElasticNetCV(cv=cv, l1_ratio=self.rho, n_alphas=10).fit(
                X, Y
            )
    ...
    def _perform_checks(self, X, Y, Attacks, Labels):
        ...
        if self._projection_type == "vector" and Attacks.shape[1] != len(
            self._projection
        ):
            raise ValueError(
                "Projection range must be of the same size as feature size."
            )
        ...
        while self.n_iter < self.max_iter:
            ...
            for attack, label in zip(Attacks, Labels):
                ...
                while n_line_iter < self.max_lsearch_iter:
                    ...
                    if self._bounds(
                        np.array([new_attack]), np.array([label])
                    ) <= self._bounds(
                        np.array([attack]), np.array([label])
                    ) - self.sigma * eta * (
                        np.linalg.norm(d) ** 2
                    ):
                        break
            ...
            if (
                abs(self._bounds(Attacks, Labels) - self._bounds(Prev_Attacks, Labels))
                < self.epsilon
            ):
                break
            Prev_Attacks = np.array(Attacks)
    ...
    def _split_attacks(self, Attacks, Labels):
        ...
        split = np.array_split(
            usable, os.cpu_count() if os.cpu_count() < len(Labels) else len(Labels)
        )

        return [
            ([row[:-1] for row in matrix], [row[-1] for row in matrix])
            for matrix in split
        ]
    ...
    def run(self, X, Y, Attacks, Labels, projection):
        ...
        self._perform_checks(
            self._use_X, self._use_Y, np.array(Attacks), np.array(Labels)
        )
    ...
    def autorun(self, X, Y, num_attacks, projection, return_initial=False):
        ...
        Attacks = random.sample(
            [x + [y] for x, y in zip(X_np.tolist(), Y_np.tolist())], num_attacks
        )
...
class frederickson2018(xiao2018):
    ...
    def _distance_threshold(self, X, Y):
        ...
        distances, _ = k_nearest_model.kneighbors(
            X, n_neighbors=1
        )  # why is n_neighbors 1

        return np.array(
            [float("inf") if distance > self.phi else 0 for distance in distances]
        )

    def _k_nearest(self, X):
        print("here2")
        print(f"X: {X}")
        euclideanDistance, k_nearest_indices = self._Knearest_neighbor.kneighbors(
            X, n_neighbors=self.k_th
        )
    ...
    def _k_partial(self, X, ax):
        ...
        partial_outlier = (
            self.power
            * (outlier_term ** (self.power - 2))
            * (ax - X[indices[0, self.k_th - 1]])
        )
    ...
    def _bounds(self, X, Y):
        ...
        return (
            (1 / length)
            * sum([(1 / 2) * ((np.dot(weights, x) - y) ** 2) for x, y in zip(X, Y)])
            + (self.alpha * self._regularize(weights))
            - (self.phi * outlier_ax)
        )
    ...
    def _gradient(self, X, Y, ax, ay):
        ...
        return ((sum(result) / X.shape[0]) + last_term) - (
            self.phi * self._useable_partials(X, ax)
        )
