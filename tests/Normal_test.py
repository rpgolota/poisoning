...
def tmark(*args, label=[], mark=pytest.mark.xfail, reason=None):
    return [
        i if i not in label else pytest.param(*i, marks=mark(reason=reason))
...
@pytest.mark.parametrize(
    "file,model",
    tmark(
        find_inputs("Input_test"),
        [xiao2018, frederickson2018],
        label=[("Input_test_1.json", frederickson2018)],
        mark=pytest.mark.skip,
        reason="Too complicated to refactor.",
    ),
)
...
@pytest.mark.parametrize("size", [1, 5, 10, 20])
@pytest.mark.parametrize(
    "proj_type", ["scalar", "tuple", "vectorscalar", "vectortuple"]
)
@pytest.mark.parametrize("model", [xiao2018, frederickson2018])
def test_project(proj_type, size, model):
    ...
    switch = {
        ...
        "vectortuple": [
            (random.randint(-10, 10), random.randint(-10, 10)) for i in range(size)
        ],
    }
    test = model()
